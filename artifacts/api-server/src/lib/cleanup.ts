...
async function runCleanup(): Promise<void> {
  ...
  try {
         ...
         AND id NOT IN (
               SELECT DISTINCT user_id
                ...
                WHERE user_role = 'client'
                  AND delivered_at > NOW() - INTERVAL '${STALE_DAYS} days'
             )
      RETURNING id
    `);
    ...
    const total =
      clientResult.rowCount +
      driverResult.rowCount +
      adminResult.rowCount;

    if (total > 0) {
      logger.info(
        {
          clients: clientResult.rowCount,
          drivers: driverResult.rowCount,
          admins: adminResult.rowCount,
        },
